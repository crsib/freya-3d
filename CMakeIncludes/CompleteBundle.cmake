...
FUNCTION( MakeCompleteBundle target )
	MESSAGE (STATUS "Fixing up bundle ${target}")
	IF(APPLE)
		SET(BUNDLE_LOCATION ${CMAKE_INSTALL_PREFIX}/${target}.app)
	ELSE()
		SET(BUNDLE_LOCATION ${CMAKE_INSTALL_PREFIX}/${target})
	ENDIF()
	MESSAGE (STATUS "Bundle is located at ${BUNDLE_LOCATION}") 
	IF(APPLE)
		SET( BUNDLE_CONTENTS ${BUNDLE_LOCATION}/Contents )
	ELSE()
		SET( BUNDLE_CONTENTS ${BUNDLE_LOCATION} )
	ENDIF()
	IF(NOT WIN32 AND NOT EXISTS ${BUNDLE_CONTENTS}/Libraries)
		FILE(MAKE_DIRECTORY ${BUNDLE_CONTENTS}/Libraries )
	ENDIF()
		SET( BUNDLE_FRAMEWORKS ${BUNDLE_CONTENTS}/Libraries)
	
	IF(UNIX)
		FILE( GLOB_RECURSE PLUGINS "${BUNDLE_CONTENTS}/Plugins/*.so" )
	ELSE()
		FILE( GLOB_RECURSE PLUGINS "${BUNDLE_CONTENTS}/Plugins/*.dll" )
	ENDIF()	
	MESSAGE(STATUS "Following plugins found: ${PLUGINS}" )
	
	if(NOT WIN32)
		SET ( SEARCH_DIRECTORIES "/usr/lib" "/usr/local/lib" ${CMAKE_INSTALL_PREFIX}/Shared)
	else()
		get_filename_component(binpath ${CMAKE_COMMAND} PATH)
		SET ( SEARCH_DIRECTORIES ${CMAKE_INSTALL_PREFIX}/Shared ${binpath} )
	endif()
	IF(APPLE)
		fixup_bundle(${BUNDLE_LOCATION} "${PLUGINS}" "${SEARCH_DIRECTORIES}")
	ELSE(APPLE)
		IF (UNIX)
			#get_target_property ( exe ${target} LOCATION)
			get_prerequisites("${BUNDLE_LOCATION}/${target}" prereqs 1 1 ${BUNDLE_LOCATION} "${SEARCH_DIRECTORIES}")
			MESSAGE(STATUS "Executable prerequisities: ${prereqs}")
			foreach(plugin ${PLUGINS})
				set( libprereqs )
				get_prerequisites("${plugin}" libprereqs 1 1 ${BUNDLE_LOCATION} "${SEARCH_DIRECTORIES}")
				SET( prereqs ${prereqs} ${libprereqs} )
				LIST( REMOVE_DUPLICATES prereqs )
			endforeach()
			MESSAGE(STATUS "Full prerequisities: ${prereqs}")
			foreach(dep ${prereqs})
				get_filename_component( depname ${dep} NAME )
				copy_resolved_item_into_bundle(${dep} ${BUNDLE_LOCATION}/Libraries/${depname}) 
			endforeach()
			MESSAGE(STATUS "Creating startup script...")
			FILE(WRITE ${BUNDLE_LOCATION}/${target}.sh "#!/bin/sh\n\ncd ${BUNDLE_LOCATION}\nLD_LIBRARY_PATH=./Libraries ./${target}\n" )
			EXECUTE_PROCESS(COMMAND chmod a+x ${BUNDLE_LOCATION}/${target}.sh)
		ELSE()
			get_prerequisites("${BUNDLE_LOCATION}/${target}.exe" prereqs 1 1 ${BUNDLE_LOCATION} "${SEARCH_DIRECTORIES}")
			MESSAGE(STATUS "Executable prerequisities: ${prereqs}")
			foreach(plugin ${PLUGINS})
				set( libprereqs )
				get_prerequisites("${plugin}" libprereqs 1 1 ${BUNDLE_LOCATION} "${SEARCH_DIRECTORIES}")
			...
			foreach(dep ${prereqs})
				unset ( depname )
				get_filename_component( depname ${dep} NAME )
				MESSAGE(STATUS "Searching for ${depname} (${dep})" )
				unset( depf CACHE)
				
				find_file(depf ${depname} ${SEARCH_DIRECTORIES} )
				MESSAGE("Full path is: ${depf}")
				copy_resolved_item_into_bundle(${depf} ${BUNDLE_LOCATION}/${depname}) 
			endforeach()
			MESSAGE("${SEARCH_DIRECTORIES}")
		ENDIF()		
	ENDIF(APPLE)
	
ENDFUNCTION( MakeCompleteBundle )
