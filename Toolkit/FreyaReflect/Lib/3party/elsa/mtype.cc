                                       ...
                                       E_variable const *pat, MatchFlags flags)
{
  ...
  Binding *binding = find(bindings, vName);
  if (binding) {
...
bool IMType::addBinding(StringRef name, Binding * /*owner*/ value, MatchFlags flags)
{
  if (flags & MF_ISOMORPHIC) {
    ...
    for (BindingMap::iterator iter = bindings.begin(); iter != bindings.end();
         iter++) {
      if (value->operator==(*iter->second)) {
  ...
  bindings.insert(BindingMap::value_type(name, value));
  return true;
}
                                    ...
                                    CVFlags tvCV, MatchFlags flags)
{
  ...
  Binding *binding = find(bindings, tvName);
  if (binding) {
...
string MType::bindingsToString() const
{
  ...
  stringBuilder sb;
  sb << "; bindings:";
  for (BindingMap::const_iterator iter = bindings.begin(); iter != bindings.end();
       iter++) {
    sb << " \"" << iter->first.c_str() << "\"=`" << iter->second->asString() << "'";
  }
  return sb;
}
...
STemplateArgument MType::getBoundValue(StringRef name, TypeFactory &tfac) const
{
  ...
  BindingMap::const_iterator iter = bindings.find(name);
  const Binding *b = (iter == bindings.end() ? NULL : iter->second);
  if (!b) {
...
void MType::setBoundValue(StringRef name, STemplateArgument const &value)
{
  ...
  Binding *tmp = find(bindings, name);
  if (tmp)
    delete tmp;
  bindings.insert(BindingMap::value_type(name, b));
}
