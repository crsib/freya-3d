...
static const float e    		= 2.7182818285f;
static float eps  	 			= 1e-6;
//static float wu_coeff			= 0.34375;
static const float infinity		= 1e15;
static const float minfinity	= -1e15;
...
inline float	atan(float _)
{
	...
	__m128	xmm4	= _mm_rcp_ss		( xmm0 ); // rcpss	xmm4, xmm0
	...
	if(n)
	{
		...
		xmm5	= _mm_rcp_ss		( xmm5 );	//rcpss	xmm5, xmm5
	...
	else
	{
		...
		__m128 	xmm7	= _mm_load_ss		( _ps_atan_s1 );//movss	xmm7, _ps_atan_s1
		xmm1	= _mm_rcp_ss		( xmm1 );//rcpss	xmm1, xmm1
