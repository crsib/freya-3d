...
int doBidi(BLOCKTYPE line, int count, int applyShape, int reorderCombining, int * v2l, int * l2v)
{
   ...
   types = (unsigned char*)core::memory::Allocate(sizeof(unsigned char) * count,core::memory::CEGUI_POOL);
   levels = (unsigned char*)core::memory::Allocate(sizeof(unsigned char) * count,core::memory::CEGUI_POOL);
   if(applyShape)
	   shapeTo = (CHARTYPE*)core::memory::Allocate(sizeof(CHARTYPE) * count,core::memory::CEGUI_POOL);
   ...
   if(applyShape)
       ...
       for(i=0; i<count; i++)
	 {
       ...
       core::memory::Free(shapeTo,core::memory::CEGUI_POOL);
     }
   ...
   core::memory::Free(types,core::memory::CEGUI_POOL);
   core::memory::Free(levels,core::memory::CEGUI_POOL);
