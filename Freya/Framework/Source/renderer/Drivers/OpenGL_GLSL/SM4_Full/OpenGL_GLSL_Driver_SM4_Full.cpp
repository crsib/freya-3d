...
#include "renderer/Drivers/OpenGL_GLSL/SM4_Full/DriverSubsystems/OpenGL_GLSL_Texture_SM4_Full.h"
#include "renderer/Drivers/OpenGL_GLSL/SM4_Full/DriverSubsystems/OpenGL_GLSL_VertexBufferObject_SM4_Full.h"
#include "renderer/Drivers/OpenGL_GLSL/SM4_Full/DriverSubsystems/OpenGL_GLSL_Framebuffer_SM4_Full.h"
#include "renderer/Drivers/OpenGL_GLSL/SM4_Full/DriverSubsystems/OpenGL_GLSL_Shader_SM4_Full.h"
...
OpenGL_GLSL_Driver::OpenGL_GLSL_Driver()
{
	...
	assert_extension(ARB_texture_non_power_of_two);//+
	assert_extension(ARB_draw_buffers); //+
	assert_extension(ARB_texture_rectangle);//+
	assert_extension(ARB_texture_float);//+
	assert_extension(ARB_pixel_buffer_object);//+
	assert_extension(SGIS_generate_mipmap);//+
	assert_extension(EXT_texture_compression_s3tc);//+
	assert_extension(EXT_framebuffer_object);//+
	assert_extension(EXT_fog_coord);
	assert_extension(EXT_packed_depth_stencil);
	assert_extension(EXT_geometry_shader4);
	assert_extension(EXT_gpu_shader4);
	m_Futures = futures::MULTITEXTURE	|
	futures::CUBE_MAP 					|
	futures::RECTANGLE_TEXTURE 		|
	futures::TEXTURE_3D					|
	futures::FLOAT_TEXTURE				|
	futures::NPOT_TEXTURE				|
...
{
	return EString("OpenGL GLSL SM40 MRT DS");
}
