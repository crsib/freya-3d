  ...
  CompoundType *ct)    // the class of which 'func' is a member
{
  ...
  while (!dtorStmtsReverse.empty()) {
    dtorStatements->append(pop_stack(dtorStmtsReverse));
  }
...
bool E_delete::elaborate(ElabVisitor &env)
{
  ...
  // dmandelin -- 3.5.3.1 and Oink ticket #32
  // It looked insanely hard to insert AST nodes for the implicit
  // conversion operator, so instead we recheck for that condition
  // here. We'll punt on the result for now because we don't care
  // too much and it's still an improvement.
  PointerType *to;
  if (t->isPointerType()) {
    to = t->asPointerType();
  } else {
