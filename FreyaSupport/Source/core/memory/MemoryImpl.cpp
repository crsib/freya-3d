...
#define FREE_BLOCK_MARK 0xDEADBEEA
#define ALLOCATED_BLOCK 0xFEEDFACE
			...
			public:
					...
					m_Alignment(alignment),m_AllocCount(0), m_Allocated(0), m_ParentPool(pool),m_HeaderOffset(0),m_Mutex(0)
				{
...
#else
					m_Buffer =  reinterpret_cast<uint8_t*>(s3eMalloc(m_Size));
#endif
					...
					if(m_Buffer)
					{
						...
						FREYA_SUPPORT_ASSERT((reinterpret_cast<uint32_t>((m_First + sizeof(__MemoryHeader))) % m_Alignment) == 0, "Invalid alignment" );
					...
					else
					{
						...
						//CriticalLog("Failed to allocate memory from the system pool");
						assert(false && "OS has refused to allocate the memory");
				...
				allocate(uint32_t size)
				{
					...
					if(!m_First || !m_Last)
						return NULL;
					//Update the size
					size += sizeof(__MemoryHeader) + m_HeaderOffset; //Size, required for headers
				...
				bool					free(uint8_t* p)
				{
					if( (p < m_Buffer) || (p >= m_Buffer + m_Size) )
						return false;
					MemoryHeader*	block = reinterpret_cast<MemoryHeaderPtr>((p - sizeof(__MemoryHeader)));
			...
			private:
				...
				inline
				uint8_t *				sliceAndRemove(MemoryHeader*  block,uint32_t size)
				{
					if(block->magic == FREE_BLOCK_MARK)
					{
...

						{
							...
							else //Slicing needed
							{
								...
								FREYA_SUPPORT_ASSERT((reinterpret_cast<uint32_t>((reinterpret_cast<uint8_t*>(slice) + sizeof(__MemoryHeader))) % m_Alignment) == 0, "Invalid alignment returned" );
								slice->size = block->size - (sizeof(__MemoryHeader) + size + m_HeaderOffset);

								FREYA_SUPPORT_ASSERT((reinterpret_cast<uint8_t*>(slice) - (reinterpret_cast<uint8_t*>(block) + sizeof(__MemoryHeader))) >= static_cast<ptrdiff_t>(size), "Invalid block size" );
						...
						// Now, drop the links. This should clean up the logics of allocator a bit
						// The price is only a couple of clocks, as soon as "block" is likely to be resident in cache
						block->next = block->prev = NULL;
						FREYA_SUPPORT_ASSERT(m_Allocated <= m_Size, "Invalid buffer size");
						FREYA_SUPPORT_ASSERT(reinterpret_cast<uint32_t>(reinterpret_cast<uint8_t*>(block) + sizeof(__MemoryHeader)) % m_Alignment == 0, "Invalid block alignment");
						FREYA_SUPPORT_ASSERT(block->size >= size, "Invalid block size");
						FREYA_SUPPORT_ASSERT(!m_Last || !(m_Last->next), "Invalid memory manager state detected");
						FREYA_SUPPORT_ASSERT(!m_First ||!(m_First->prev), "Invalid memory manager state detected");
						FREYA_SUPPORT_ASSERT(m_First != block, "Invalid memory manager state detected");
						FREYA_SUPPORT_ASSERT(m_Last != block, "Invalid memory manager state detected");
#ifdef MEMORY_DEBUGGING_ENABLED
						memset(reinterpret_cast<uint8_t*>(block) + sizeof(__MemoryHeader), NULL, block->size);
#endif
				...
				void					insertAndConnect(MemoryHeaderPtr block)
				{
					...
					}
					{
						...
						m_AllocCount--;
						assert(static_cast<int32_t>(m_AllocCount) >= 0);
						assert(m_Allocated > block->size);
						m_Allocated -= block->size;
#ifndef USES_QT
						assert(memory_allocated >= block->size && "Invalid allocator state");
#else
						if(memory_allocated < block->size)
						{
							brk();
							abort();
						}
						...
						else//!((m_First == m_Last) && (m_First == block))
						{
							...
							{
								block->next = NULL;
								if(m_Last && ((reinterpret_cast<uint8_t*>(m_Last) + m_Last->size + m_HeaderOffset + sizeof(__MemoryHeader)) == reinterpret_cast<uint8_t*>(block)))
								{
								...
								else
								{
									...
									{
										m_First = block;
										//block->prev = NULL; // This basically means, tha
									}
							...
							else //General variant m_First < block < m_Last
							{
								...
								FREYA_SUPPORT_ASSERT(prev_blc, "Invalid previous block");
						...
						//if((block->prev == NULL) /*&& (block != m_First)*/)
						//	m_First = block;
						//assert(!(block->next == NULL /*&& block != m_Last*/));
						//if((block->next == NULL) /*&& (block != m_Last)*/)
						//	m_Last = block;

						if((m_Last->next) || (m_First->prev))
						{
							ErrorLog("");
						}

						FREYA_SUPPORT_ASSERT(!(m_Last->next), "Invalid memory manager state detected");
						FREYA_SUPPORT_ASSERT(!(m_First->prev), "Invalid memory manager state detected");

						assert(static_cast<int32_t>(m_Allocated) >= 0);
					} // End of sync
					m_Mutex = 0;
				}
 ...
 ************************************************************************************************/
			...
			public:
				...
				bool init(int fixedAllocSize, int bdIndex, int chunks)
				{
					...
					FREYA_SUPPORT_ASSERT(reinterpret_cast<uint32_t>(mDataStart)%16 == 0, "Invalid small block allocator state");
				...
				void* alloc()
				{
					...
					{
#ifdef MEMORY_DEBUGGING_ENABLED
						if((char*)mFreeChunk < ((char*)this + getHeaderSize(0)) || ((unsigned char*)mFreeChunk >= mLastByte))
						{
							//ErrorLog("Small block pool is trashed");
							mFreeChunk = NULL;
...
#ifdef MEMORY_DEBUGGING_ENABLED
						if(mFreeChunk&&((char*)mFreeChunk < ((char*)this + getHeaderSize(0)) || ((unsigned char*)mFreeChunk >= mLastByte)))
						{
							//ErrorLog("Small block pool is trashed");
							mFreeChunk = NULL;
						}
				...
				inline void free(void* p)
				{
					...
					*pp = mFreeChunk;
					mFreeChunk = (void**)p;
					assert((char*)mFreeChunk >= ((char*)this + getHeaderSize(0)));
					assert((unsigned char*)mFreeChunk <= mLastByte);
					mAllocCount--;
				}
 ...
 ************************************************************************************************/
			...
			public:
					...
					m_PoolId(poolId), m_PreallocSize(preallocSize),m_Alignment(alignment),m_Lock(0)
				{
					...
					FREYA_SUPPORT_ASSERT(buf, "Failed to allocate memory buffer");
					if(buf->isOk())
					{
				...
				{
					void* p;
					if((sz <= 256) && (p = m_SmallBlockPool->alloc(sz)))
					{
						//m_Lock = 0;
						return p;
					}
				...
				allocate(size_t sz)
				{
					...
					m_Buffers.push_back(new MemoryBuffer((pr > sz ? pr : sz) + sizeof(__MemoryHeader) + 16 ,m_Alignment,m_PoolId));
					m_Lock = 0;
...
#define ALLOCJR_FULLBLOCK (block*)0xffffffff
					...
					if (mFreeBlocks[bdIndex])
					{
						...
						block *b = mFreeBlocks[bdIndex];
							...
							if(result)
							{
								allocation_count++;
								alloc_dealloc_dif++;
								memory_allocated += b->mFixedAllocSize;
							}
#endif
			...
			bool rtAllocator::free(void* p)
			{
				...
				if (b)
				{
					...
					deallocation_count++;
					alloc_dealloc_dif--;
					assert(memory_allocated >= b->mFixedAllocSize&&"Invalid allocator state");
					memory_allocated -= b->mFixedAllocSize;
#endif
			...
			void rtAllocator::addBlockToArray(block* b)
			{
				...
				while (s < e)
				{
					...
					else if (mBlockArray[m] && ((unsigned char*)b > mBlockArray[m]->getLastByte()))
					{
						s = m+1;
					}
					m = s+ ((e-s+1)/2);
				}
				if (s<mBlockCount && (mBlockArray[s] && ((unsigned char*)b > (unsigned char*)mBlockArray[s]->getLastByte())))
				{
					s++;
				}
 ...
 ************************************************************************************************/
		...
		MemoryArena::MemoryArena()
		{
			...
			m_Pools.push_back(new __internal::MemoryPool(DEFAULT_POOL,128,4)); //ARM cpu possibly will not support unaligned addresses
			m_Pools.push_back(new __internal::MemoryPool(STL_POOL,1024*1024,4));
			m_Pools.push_back(new __internal::MemoryPool(MATH_POOL,1024*1024,4));
			m_Pools.push_back(new __internal::MemoryPool(GENERIC_POOL,1024*2024,4));
			m_Pools.push_back(new __internal::MemoryPool(CLASS_POOL,1024*1024,4));
			m_Pools.push_back(new __internal::MemoryPool(LUA_POOL,1024*1024,4));
		...
		MemoryArena::~MemoryArena()
		{
			...
			assert(!(allocation_count - deallocation_count)|| "Memory leak detected");
		}

		MemoryArena*	MemoryArena::GetInstance()
		{
			static    MemoryArena __;
			return &__;
...
#ifdef  MEMORY_DEBUGGING_ENABLED
		int MemoryArena::getOwner( void* p )
		{
...
extern "C"
{
	bool  check_overruns( void* _blk, size_t size)
	{
		using namespace core::memory;
		using namespace core::memory::__internal;
		
		uint8_t* ptr = reinterpret_cast<uint8_t*>(_blk);

		for( size_t i = 0; i < size; ++i )
		{
			MemoryHeader* header = reinterpret_cast<MemoryHeaderPtr>(ptr);
			if( (header->magic == FREE_BLOCK_MARK) || (header->magic == ALLOCATED_BLOCK) )
				return true;
			++ptr;
		}
	...
	void* memcpy_debug( void* dest, const void* source, size_t num )
	{
		assert(!check_overruns(dest, num));
		return memcpy(dest,source,num);
	}

	void* memmove_debug( void * destination, const void * source, size_t num )
	{
		assert(!check_overruns(destination, num));
		return memmove(destination, source, num);
	}

	void* memset_debug ( void * ptr, int value, size_t num )
	{
		assert(!check_overruns(ptr, num));
		return memset(ptr, value, num);
	}

	char* strcpy_debug( char * destination, const char * source )
	{
		assert(!check_overruns(destination, strlen(source)));
		return strcpy(destination, source);
	}

	char* strncpy_debug( char * destination, const char * source, size_t num )
	{
		assert(!check_overruns(destination, num));
...
// This will pre-compile standard object definition to silence the linker of Visual Studio
