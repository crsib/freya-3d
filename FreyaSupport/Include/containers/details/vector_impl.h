...
namespace containers
{
	...
	{
		m_End = details::__vector_impl_copy_helper(_begin,_end, m_Begin,eastl::has_trivial_copy<T>());
	}
		...
		void containers::vector<T, MemoryAllocationPolicy, LockPolicy, StorageResizePolicy>::insert( iterator position, const_iterator first, const_iterator last )
	{
		...
		if(!m_Begin)
		{
			...
			m_End = details::__vector_impl_copy_helper( first, last, m_Begin, eastl::has_trivial_copy<T>());
		}
		else if( (m_End - m_Begin) > static_cast<ptrdiff_t>(m_AllocatedCount - elem_count) )
		{
			...
			if(position <= m_Begin)
			{
				details::__vector_impl_copy_helper( first, last, new_memory_ptr,  eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr + elem_count, eastl::has_trivial_copy<T>());
			}
			else if(position >= m_End)
			{
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr, eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(first, last, m_End, eastl::has_trivial_copy<T>());
			}
			else //Worst case
			{
				T* mem = details::__vector_impl_copy_helper(m_Begin, position, new_memory_ptr, eastl::has_trivial_copy<T>());
				mem = details::__vector_impl_copy_helper( first, last, mem, eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(position, m_End, mem, eastl::has_trivial_copy<T>());
			}
			deallocate(m_Begin);
		...
		else
		{
			if(position <= m_Begin)
			{
				...
				details::__vector_impl_copy_helper( first, last, m_Begin, eastl::has_trivial_copy<T>());
			}
			else if(position >= m_End)
			{
				m_End = details::__vector_impl_copy_helper( first, last, m_End, eastl::has_trivial_copy<T>());
			...
			else
			{
				...
				details::__vector_impl_copy_helper( first, last, position, eastl::has_trivial_copy<T>());
			}
		}
		unlock();
	} //void containers::vector<T, MemoryAllocationPolicy, LockPolicy, StorageResizePolicy>::insert( iterator position, const_iterator first, const_iterator last )
		...
		void containers::vector<T, MemoryAllocationPolicy, LockPolicy, StorageResizePolicy>::insert( iterator position, size_t n, constant_refernce obj )
	{
		...
		else if( (m_End - m_Begin) > static_cast<ptrdiff_t>(m_AllocatedCount - n) )
		{
			...
			if(position <= m_Begin)
			{
				...
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr + n, eastl::has_trivial_copy<T>());
			}
			else if(position >= m_End)
			{
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr, eastl::has_trivial_copy<T>());
		...
		void containers::vector<T, MemoryAllocationPolicy, LockPolicy, StorageResizePolicy>::insert( iterator position, constant_refernce obj )
	{
		...
		if(!m_Begin)
		{
			...
			m_Begin = allocate(m_AllocatedCount);
			m_End = details::__vector_impl_copy_helper(m_Begin,obj,eastl::has_trivial_copy<T>());
		}
		else if( (m_End - m_Begin) == m_AllocatedCount )
		{
			...
			if(position <= m_Begin)
			{
				details::__vector_impl_copy_helper(new_memory_ptr, obj, eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr + 1, eastl::has_trivial_copy<T>());
			}
			else if(position >= m_End)
			{
				m_End = details::__vector_impl_copy_helper(m_Begin, m_End, new_memory_ptr, eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(m_End, obj, eastl::has_trivial_copy<T>());
			}
			else //Worst case
			{
				T* mem = details::__vector_impl_copy_helper(m_Begin, position, new_memory_ptr, eastl::has_trivial_copy<T>());
				mem = details::__vector_impl_copy_helper(mem, obj, eastl::has_trivial_copy<T>());
				m_End = details::__vector_impl_copy_helper(position, m_End, mem, eastl::has_trivial_copy<T>());
			}
		...
		else
		{
			if(position <= m_Begin)
			{
				...
				details::__vector_impl_copy_helper(m_Begin, obj, eastl::has_trivial_copy<T>());
			}
			else if(position >= m_End)
			{
				m_End = details::__vector_impl_copy_helper(m_End, obj, eastl::has_trivial_copy<T>());
			...
			else //Worst case
			{
				...
				details::__vector_impl_copy_helper(position, obj, eastl::has_trivial_copy<T>());
			}
		}
		unlock();
	} // void containers::vector<T, MemoryAllocationPolicy, LockPolicy, StorageResizePolicy>::insert( iterator position, constant_refernce obj )
